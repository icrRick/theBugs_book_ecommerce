...
public interface StatisticalProductJPA extends JpaRepository<Product, Integer> {
    ...
    @Query("""
            SELECT new com.thebugs.back_end.dto.Statistical_DTO.Child_Chart_DTO(g.name, COUNT(p.id))
            FROM Product p
            ...
            GROUP BY g.name
            """)
    List<Child_Chart_DTO> getChartDataGenresProductByShopId(@Param("shopId") int shopId);
    ...
    @Query("""
                SELECT
                    p.name, p.product_code, GROUP_CONCAT(DISTINCT g.name), COALESCE(SUM(oi.quantity), 0), p.quantity, COALESCE(SUM(oi.quantity * oi.price), 0)
                    FROM Product p
            ...
            """)
    List<Object[]> getProductSummaryByShopId(@Param("shopId") int shopId);
