...
const Stores = () => {
  ...
  const configMessage = (item) => {
    ...
    return <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item
  ...
  const handleEdit = (item) => {
    navigate(`/admin/shop/${item.shopSlug}`);
  };

  const handleRefresh = () => {
    fetchItems(keyword, currentPage);
    showSuccessToast('Đã làm mới danh sách cửa hàng');
  };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center text-sm text-gray-500 mb-1">
                  ...
                  <span className="text-gray-700 font-medium">Quản lý cửa hàng</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">Quản lý cửa hàng</h1>
                    <p className="text-gray-500 text-xs sm:text-sm mt-1">Quản lý tất cả cửa hàng trong hệ thống</p>
                  </div>
                </div>
              </div>
