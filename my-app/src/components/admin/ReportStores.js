...
const ReportStores = () => {
  ...
  const fetchData = async (active, page) => {
    try {
      ...
      const response = await axiosInstance.get(`/admin/report/shop/list?active=${active}&page=${page}`);
  ...
  const handleTabChange = (tab) => {
    ...
    navigate(`/admin/reports/stores?${params.toString()}`);
    fetchData(params.get("active"), 1);
  };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center text-sm text-gray-500 mb-1">
                  ...
                  <span className="text-gray-700 font-medium">Quản lý báo cáo cửa hàng</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">Quản lý báo cáo cửa hàng</h1>
                    <p className="text-gray-500 text-xs sm:text-sm mt-1">Quản lý tất cả báo cáo cửa hàng trong hệ thống</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex justify-end">
                    <div className="flex gap-2">
                      <Link
                        to={`/admin/reports/stores/${report?.id}`}
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4 transform transition-all duration-300 scale-100">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 mb-2">
                Bạn có chắc chắn muốn duyệt báo cáo cửa hàng "{selectedItem?.productName || ''}" không?
              </p>
