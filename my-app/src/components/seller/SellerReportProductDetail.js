...
const SellerReportProductDetail = () => {
  const { id } = useParams();
  ...
  const fetchReportDetail = async (id) => {
    ...
    try {
      const response = await axiosInstance.get(
        `/seller/report/product/detail?id=${id}`
      );
        ...
        <div className="flex items-start space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Email người dùng: {report?.emailUser}
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
              {report?.images.map((image, index) => (
                <div
                  key={index}
                  className="relative group overflow-hidden rounded-lg shadow-sm hover:shadow-md transition-all duration-300 bg-gray-50"
                >
                  <img
...
export default SellerReportProductDetail;
