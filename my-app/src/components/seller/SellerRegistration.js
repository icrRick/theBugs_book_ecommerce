...
const SellerRegistration = () => {
  ...
  const handleShopInfoChange = (e) => {
    ...
    if (type === "file" && (name === "shopLogo" || name === "shopBanner")) {
      setShopInfo((prev) => ({
        ...prev,
        [name]: files[0],
      }));
  ...
  const handleIdRecognitionData = (data) => {
    if (data) {
      setIdRecognition((prev) => ({
        ...
        idIssueDate: convertToDate(data.issue_date) || prev.idIssueDate, // Chỉnh sửa trường "issue_date"
        ...
        addressEntities: data.address_entities || prev.addressEntities, // Thêm trường "address_entities"
    ...
    return (
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8 text-center">
        ...
        <p className="text-gray-600 mb-8">
          ...
          Chúng tôi sẽ xem xét thông tin của bạn và thông báo kết quả trong thời
          gian sớm nhất.
        </p>
        <p className="text-gray-500">
          Bạn sẽ được chuyển hướng đến trang quản lý sau vài giây...
        </p>
      </div>
    );
  }
