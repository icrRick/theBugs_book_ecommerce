...
const EditPromotion = () => {
  ...
  const [oldPromotionProduct, setOldPromotionProduct] = useState([]);
  ...
  useEffect(() => {
    const updatedInfo = selectedProducts.map((selected) => {
      const product = products.find((p) => Number(p.id) === selected.id) || {};
      ...
      return {
        ...
        currentPromotionDTO: {
          value: product.currentPromotionDTO?.value ?? null,
          startDate: product.currentPromotionDTO?.startDate ?? null,
          endDate: product.currentPromotionDTO?.endDate ?? null,
          isActive: product.currentPromotionDTO?.isActive ?? null,
        },
      };
    });
    setSelectedProductsInfo(updatedInfo);
  ...
  useEffect(() => {
    console.log("Selected Products Info:", selectedProductsInfo);
  ...
  const getPromotionDetails = async () => {
    ...
    axiosInstance
      .get(`/api/seller/promotion/${promotionId}`)
      .then((res) => {
        setFormData(mapPromotionData(res.data));
        if (res.data.promotionProductDTO) {
          setOldPromotionProduct(res.data.promotionProductDTO);
          setSelectedProducts(res.data.promotionProductDTO);
          setSelectedProductsInfo(res.data.promotionProductDTO);
        }
      })
      .catch((error) => {
  ...
  const handleAddProduct = () => {
    ...
    if (product.currentPromotionDTO && formData.startDate) {
      ...
      // Kiểm tra xem sản phẩm trong danh sách oldProduct có trùng với newProductId không
      const isProductInOldList = oldPromotionProduct.some(
        (oldProductItem) => oldProductItem.id === newProductId
      );

      if (!isProductInOldList) {
  ...
  const handleSubmit = (e) => {
    ...
    axiosInstance
      .put(
        isEditing
          ? `/api/seller/promotion/update/${promotionId}`
          : "/api/seller/promotion/create",
        dataToSend
      )
      .then(() => {
        showSuccessToast(
          isEditing
            ...
            : "Khuyến mãi đã được thêm thành công"
        );
