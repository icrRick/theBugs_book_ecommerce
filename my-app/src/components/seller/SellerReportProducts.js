...
const SellerReportProducts = () => {
  ...
  const fetchData = async (active, page) => {
    try {
      const response = await axiosInstance.get(
        `/seller/report/product/list?active=${active}&page=${page}`
      );
      ...
      showErrorToast(
        "Có lỗi xảy ra khi tải dữ liệu báo cáo" + error.response?.data?.message
      );
    }
  };
  ...
  const handleTabChange = (tab) => {
    ...
    navigate(`/seller/report-products?${params.toString()}`);
    fetchData(tab, 1);
  };

  const handleViewDetails = (report) => {
    // Save the current active tab and page in sessionStorage before navigating
    sessionStorage.setItem("previousTab", activeTab);
    sessionStorage.setItem("previousPage", currentPage.toString());
    navigate(`/seller/report-product/${report.id}`, {
      state: { from: "report-list" },
    });
  };
  ...
  useEffect(() => {
    ...
    const { activeValue, page } = getQueryParams();

    // If returning from detail view, restore previous state
    const previousTab = sessionStorage.getItem("previousTab");
    const previousPage = sessionStorage.getItem("previousPage");

    if (window.history.state?.state?.from === "report-list" && previousTab) {
      setActiveTab(previousTab);
      setCurrentPage(previousPage ? Number(previousPage) : 1);
      fetchData(previousTab, previousPage || 1);
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="-mb-px flex space-x-6">
          <button
            ...
            className={`whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-all duration-300 ease-in-out ${
              activeTab === "all"
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium shadow-sm ${
                            getStatusInfo(report?.active).color
                          }`}
                        >
                      ...
                      {report?.active !== null && report?.approvalDate && (
                        <div className="flex items-center text-xs text-gray-500">
                          <span className="mr-1">
                            Ngày {report?.active ? "duyệt" : "từ chối"}:
                          </span>
                          <svg
                            className="w-3 h-3 mr-1 text-gray-400"
                            fill="none"
...
export default SellerReportProducts;
